...
RSpec.describe 'dives show page', type: :feature do
    ...
    visit "/dives/#{dive_1.id}"

    expect(current_path).to eq("/dives/#{dive_1.id}")
  ...
  describe "Dive Update" do
    ...
    it "I see a link to update that Dive 'Update Dive'- '/dives/:id'" do
      visit("/dives/#{@coral_sea_trench.id}")
      expect(current_path).to eq("/dives/#{@coral_sea_trench.id}")
    ...
    it "I see a from, I edit attributes and click submit - '/dive/:id/edit'" do
      visit("/dives/#{@coral_sea_trench.id}/edit")
      expect(current_path).to eq("/dives/#{@coral_sea_trench.id}/edit")
